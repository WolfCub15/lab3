...
void TPolIZ::operand(const std::string & p) {
	if (p[0] >= '0' && p[0] <= '9') {//number
		double number = std::stod(p.c_str());
...
void TPolIZ::make_poliz(const std::string & tmp) {
	std::string zn = "()+-*/^ ";
	std::string * lex = new std::string[tmp.size() + 1];
	int cnt_lex = 0;
...
TPolIZ::TPolIZ(const TPolIZ & tmp) {
	poliz = new std::string[tmp.size_poliz];
...
std::string TPolIZ::operator[](const int k) const {
